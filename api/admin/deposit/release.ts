  ...
  try {
    ...
    if (deposit.status !== 'AUTHORIZED') {
      return res.status(400).json({
        error: 'Depósito não pode ser liberado',
        details: `Status atual: ${deposit.status}`,
      });
    }

    // Cancel the pre-authorization in PagBank
    try {
      await axios.post(
        `${PAGBANK_API_URL}/charges/${deposit.charge_id}/cancel`,
        {},
        {
          headers: {
      ...
      console.error('PagBank cancel error:', pagbankError);
      // Continue even if PagBank fails (might already be cancelled)
    }
    ...
    return res.status(200).json({
      success: true,
      message: 'Caução liberada com sucesso',
      depositId,
      status: 'RELEASED',
    });
  } catch (error) {
